 ...
 *
 * Usage: node scripts/verify-phase1-database.js
 */
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      `;
      const enumResult = await prisma.$queryRawUnsafe(enumQuery);
    ...
    try {
      ...
      `;
      const orphanResult = await prisma.$queryRawUnsafe(orphanQuery);
    ...
    try {
      const roleQuery = `
        ...
        ORDER BY slug;
      `;
      const roleResult = await prisma.$queryRawUnsafe(roleQuery);
